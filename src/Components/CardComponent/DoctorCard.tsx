...
const DoctorCard: React.FC<DoctorCardProps> = ({ name, specialty, image }) => {
  return (
    <div className="bg-white shadow-lg rounded-lg p-4 sm:p-6 flex flex-col items-center mx-auto mb-6 max-w-xs sm:max-w-sm md:max-w-md">
      <img
        src={image}
        alt={name}
      ...
      <p className="text-gray-600 font-semibold text-center text-xs sm:text-sm md:text-base">{specialty}</p>
    </div>
  );
};
