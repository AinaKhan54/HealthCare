            ...
            {tasks.map(task => (
              <li key={task.id} className="flex flex-col sm:flex-row justify-between items-center mb-2">
                ...
                <div className="flex space-x-2">
                  <button onClick={() => handleTaskEdit(task)} className="text-blue-500 hover:text-blue-700">Edit</button>
                  <button onClick={() => onDeleteTask(task.id)} className="text-red-500 hover:text-red-700">Delete</button>
                </div>
              </li>
            ))}
          </ul>
